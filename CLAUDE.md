...
- **S3 Xtensa LLVM bug (1.94.0.0–1.95.0.0, FIXED in 1.96.0.0)**: `xtensa-esp32s3-espidf` builds failed with an `XtensaISD::PCREL_WRAPPER` LLVM ICE in `serde_json::Vec` deserialization on `esp-rs/rust` releases 1.94.0.0 through 1.95.0.0 (tracked at [esp-rs/rust#277](https://github.com/esp-rs/rust/issues/277), regression from the LLVM 20→21 bump). **Resolved in 1.96.0.0** (released 2026-05-27) via the LLVM ConstantPool-lowering fix ([espressif/llvm-project@39a5993](https://github.com/espressif/llvm-project/commit/39a59933237b749f9c5b5f3adf02384e8dfec78e)) — verified building `devkitc` cleanly. **Use 1.96.0.0 or newer** (`espup install` installs the latest): no pin needed, and both boards build on a single toolchain. Only pin to 1.93.0.0 (`espup install --toolchain-version 1.93.0.0`) if you must use a toolchain in the broken 1.94–1.95 range. P4 (RISC-V) was never affected.
